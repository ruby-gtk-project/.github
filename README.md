...
| `fork-new-gtk-apps.yml` | weekly | Reads `registry.yml` and makes every listed fork exist: forks it when the upstream is on GitHub, mirrors it in when it is not. | **Creates repositories in the org directly.** No pull request, no issue. Does nothing to forks that already exist. |
...
| `scaffold-port-forks.yml` | daily, and right after `fork-new-gtk-apps.yml` creates forks | Every fork has a scaffolded `ruby` branch matching `port-scaffold/`. | **Commits and pushes to each fork's `ruby` branch directly.** No pull request. Creates the orphan branch where one is missing. |
